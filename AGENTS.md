...
See `include/ctap2.h` for full parameter signatures. Summary:
...
- `ctap2_make_credential_with_keepalive` / `ctap2_get_assertion_with_keepalive` -- keepalive callback
...
Common positive status bytes: 0x27 = operation denied, 0x2E = no credentials, 0x31 = wrong PIN, 0x32 = PIN blocked, 0x35 = PIN not set.

## Thread Safety

All exported C functions are thread-safe. Each call opens its own HID device handle and closes it after the transaction.

## Platform Requirements

**macOS:** IOKit.framework, CoreFoundation.framework. Entitlement: `com.apple.security.device.usb`. User must grant Input Monitoring.

**Linux:** hidraw device access (udev rules or root). No additional libraries.
